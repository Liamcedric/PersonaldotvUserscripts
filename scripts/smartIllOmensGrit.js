/**
 * Author: infinity
 * Name: Ill Omens Grit Auto
 * Description: Automatically uses Lesser Grit pots for you.
 */

if (JSON.parse(localStorage.getItem("scriptSettings"))?.smartIllOmensGrit) main();
...
async function usePot() {
	...
	await fetch(
		"https://api.dragonsofthevoid.com/api/usable/consume/u.ill-omens",
		{ headers: { authorization: this.localStorage.token } }
	)
		.then((res) => res.json())
		.then((data) => {
			if (data.success) {
				secondsUntilNextPot = 2700;
			} else {
